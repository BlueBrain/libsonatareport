...
std::pair<std::string,std::string> getIMEMountPoints() {
    ...
    const auto parseFile = [](std::ifstream ifs,
                              const std::string keyword,
                              const char sep) -> std::string
    {
        if (ifs.is_open()) {
            std::string line;
...
std::pair<fstype_t, std::string> IMEUtil::getPathInfo(std::string path) {
    ...
    path = std::string(full_path) + "/" +
           path.substr((limit != std::string::npos) ? limit : 0);
    ...
    if (path.find("ime") != std::string::npos) {
        ...
        if (path.find(mnt_paths.first) == 0) {
            return std::pair<fstype_t, std::string>(FSTYPE_IME_MNTP,
                                                    IME_PREFIX + path);
        }

        // Lastly, evaluate if the path is under a FUSE mount point
        struct statfs st;
        if (path.find(mnt_paths.second) == 0 &&
            statfs(path_orig.c_str(), &st) == 0 && st.f_type == FUSE_SUPER_MAGIC) {

...
FrameInfo::FrameInfo(char* buffer, char* endOfBuffer, bool isNative, CellInfo::key sortMode) {
    ...
    for (char* bufferLoc = buffer; bufferLoc < endOfBuffer; bufferLoc += SIZE_CELL_INFO_LENGTH) {
...
CellInfo ReadManager::makeNullInfo() {
    ...
    for (char *i = fakeBuffer, *bufferEnd = fakeBuffer + SIZE_CELL_INFO_LENGTH; i < bufferEnd; i++)
        *i = 0;
...
void FrameParser::FrameOrganiser::fillData(const FrameInfo& info,
    ...
    for (const DataItem* ptr = dataBuffer; pinf != info.end();
        ...
        iterator ins = insert(make_pair(*pinf, CellData(sectSort))).first;  // insert CellData
                                                                            // object for current
                                                                            // gid
...
FrameParser::CompartmentIndexing FrameParser::FrameOrganiser::makeOrderedOffsetReferences() const {
...
FrameParser::FrameParser(const string& file, bool sortData)
    : ReadManager(file) {
...
void FrameParser::createReferences(const FrameInfo& cinfo, bool sortData) {
    ...
    for (DataItem *mpptr = tempBuffer, *endptr = mpptr + pinf->getAmountOfCompartments();
         pinf != cinfo.end();
         pinf++, mpptr = endptr, endptr += pinf->getAmountOfCompartments())
    ...
    if (sortData) {
        ...
        org.makeOrderedDataBuffer(readBuffer);  // rearranges indexes to correspond to gid and
                                                // segment order
...
void FrameParser::bufferTransfer(DataItem* buffer) const {
    ...
    for (DataItem **sourcePtr = refArray,
                  *target = buffer,
...
void FrameParser::readFrameMapping(MappingItem* buffer) {
    ...
    readFrame(buffer);  // read the data - this function will also alter the filepointer and
                        // timestep
...
FrameParser::ReadOptimiser::ReadOptimiser(const FrameInfo& cellsToRead,
    ...
    for (multiset<FilePosition>::iterator currentPos = dataEdges.begin(), prevPos = currentPos++;
         currentPos != dataEdges.end();
         prevPos = currentPos, currentPos++)
    ...
    for (toggleOnRead = true, it = offsets.begin(); it != offsets.end();
        ...
        if (toggleOnRead) {
            ...
            dataSize_elements += (*nextRead /= elementSize);  // next read was still in units of
                                                              // bytes
            *prevRead += *nextRead;                           // combine nextRead with prevRead
            offsets.erase(it);
            offsets.erase(nextRead);
            it = prevRead;        // point to prevRead now
            toggleOnRead = true;  // and mark that toggle flag that current iteration deal with
                                  // read.
        }
    }
